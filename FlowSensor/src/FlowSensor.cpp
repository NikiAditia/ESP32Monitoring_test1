 ...
 */
void FlowSensor::build(uint16_t type ,uint8_t pin)
{
    this->_pin = pin;
...
{
    this->_flowratesecound = (this->_pulse / (this->_pulse1liter + calibration)) / ((millis() - this->_lastTimeFlow) / 1000);
    this->_volume += (this->_pulse / (this->_pulse1liter + calibration));
    this->_totalpulse += this->_pulse;
    this->_pulse = 0;
    this->_lastTimeFlow = millis();
}

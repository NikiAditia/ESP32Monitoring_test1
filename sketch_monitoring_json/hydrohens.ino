...
const char* ssid = "A55 milik Muhammad";
const char* password = "palabapaknya";

// WebSocket server details
const char* websocket_server = "192.168.43.219";
      ...
      if (strcmp(currentModeString, "Automatic Mode") == 0) { 
        ...
        for (int channel = 0; channel < ACTIVE_SERVO; channel++) {
          ...
          if (servoStatus[channel] == 1 && distanceValues[channel] < UPPER_MARGIN_DRINKER) {
            ...
            Serial.printf("[WaterControl] Manual Mode: Servo %d OFF automatically due to distance (%.2f cm > threshold).\n", 
                          channel, distanceValues[channel]);
          }
        }
...
void sendControlDataToWebSocket() {
    ...
    // Menambahkan tipe pesan
    jsonDoc["type"] = "control";

    // Data kontrol (misalnya status servo)
    JsonObject control_status = jsonDoc.createNestedObject("control_status");
    for (int i = 0; i < ACTIVE_SERVO; i++) {
        control_status[String("servo_") + String(i)] = servoStatus[i]; // Status servo 0 atau 1
    }

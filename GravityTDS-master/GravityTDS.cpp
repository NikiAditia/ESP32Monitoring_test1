...
void GravityTDS::setKvalue(float kValue)
{
    this->kValue = kValue;
}
...
{
    static int analogBuffer[MEDIAN_FILTER_SIZE];
    static int bufferIndex = 0;

    // Read the current analog value
    analogBuffer[bufferIndex] = analogRead(this->pin);
    bufferIndex = (bufferIndex + 1) % MEDIAN_FILTER_SIZE;

    // Copy buffer to a temporary array for sorting
    int sortedBuffer[MEDIAN_FILTER_SIZE];
    ...
    this->analogValue = sortedBuffer[MEDIAN_FILTER_SIZE / 2];
...
{
  byte modeIndex = 0;
  if(strstr(cmdReceivedBuffer, "ENTER TDS") != NULL) 
      modeIndex = 1;
  else if(strstr(cmdReceivedBuffer, "EXIT TDS") != NULL) 
      modeIndex = 3;
  else if(strstr(cmdReceivedBuffer, "CAL TDS:") != NULL)   
      modeIndex = 2;
  return modeIndex;
}
...
void GravityTDS::ecCalibration(byte mode)
{
    ...
    switch(mode)
    {
      ...
      case 2:
      cmdReceivedBufferPtr=strstr(cmdReceivedBuffer, "CAL TDS:");
      cmdReceivedBufferPtr+=strlen("CAL TDS:");
